...
class State:
    ...
    def __init__(self, copy: "State" = None, size_row=None, size_col=None):
        """
        ...
        self._hash = None
        if copy is None:
            State.MAX_ROW = size_row
            State.MAX_COL = size_col
            self.agent_row = None
            self.agent_col = None

            State.walls = [
                [False for _ in range(State.MAX_COL)] for _ in range(State.MAX_ROW)
            ]
            self.boxes = [
